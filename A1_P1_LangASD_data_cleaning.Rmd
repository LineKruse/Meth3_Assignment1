...
```{r}
token_train <- read.csv("~/Documents/Experimental Methods III/Assignment_1/token_train.csv")

LU_train <- read.csv("~/Documents/Experimental Methods III/Assignment_1/LU_train.csv", header=TRUE)

demo_train_1 <- read.csv("~/Documents/Experimental Methods III/Assignment_1/demo_train(1).csv", header=TRUE)
```
...
library(tidyverse)
library(plyr)
demo_train_1 = rename(demo_train_1, SUBJ = Child.ID)
demo_train_1 = rename(demo_train_1, VISIT = Visit)
...
```{r}
library(stringr)
LU_train$VISIT = str_extract_all(LU_train$VISIT, regex("\\d"))
token_train$VISIT = str_extract_all(token_train$VISIT, regex("\\d"))
```
...
```{r}
demo_train_1$SUBJ = gsub("\\.", "", demo_train_1$SUBJ)
...
```{r}
demo_train_1 = rename(demo_train_1, nonVerbalIQ = MullenRaw)
demo_train_1 = rename(demo_train_1, VerbalIQ = ExpressiveLangRaw)
```
...
all_1 = merge(demo_train_1, LU_train, by=c("SUBJ","VISIT"))
all = merge(all_1, token_train, by=c("SUBJ","VISIT"))

all = select(all, SUBJ, VISIT, Ethnicity, Diagnosis, Gender, Age, ADOS,  nonVerbalIQ, VerbalIQ, MOT_MLU, MOT_LUstd, CHI_MLU, CHI_LUstd, types_MOT, types_CHI, tokens_MOT, tokens_CHI)
...
```{r}
sub_visit1 = filter(all, VISIT==1)
sub_visit1 = select(sub_visit1,ADOS, nonVerbalIQ, VerbalIQ, SUBJ)

all_new = merge(all, sub_visit1, by="SUBJ")
all = select(all_new, -ADOS.x, -nonVerbalIQ.x, -VerbalIQ.x)
all = rename(all, ADOS = ADOS.y)
all = rename(all, nonVerbalIQ = nonVerbalIQ.y)
all = rename(all, VerbalIQ = VerbalIQ.y)
```
...
all$SUBJ = as.numeric(as.factor(all$SUBJ))

all$VISIT = as.numeric(all$VISIT)

library(plyr)
all$Gender = as.factor(all$Gender)
all$Gender = revalue(all$Gender, c("1"="M", "2"="F"))

all$Diagnosis = revalue(all$Diagnosis, c("A"="ASD", "B"="TD"))
```
...
```{r}
write.csv(all, "Ass1_clean_data", row.names = TRUE, na="NA", eol = "\r")
```
...
```{r}
clean <- read.csv("~/Documents/Experimental Methods III/Assignment_1/Ass1_clean_data")

MLU_high = filter(clean, CHI_MLU >=2.7)
MLU_low = filter(clean, CHI_MLU <=1.5, VISIT == 1)
NA_trials = filter(clean, VISIT == "NA")
...
```{r}
x = filter(clean, VISIT == 6)
arrange(x, desc(types_CHI))
#Answer = subject 26 

xx = filter(clean, VISIT == 1)
arrange(xx, types_CHI)
#Answer = subject 57
...
```{r}
clean_new = filter(select(clean, SUBJ, VISIT, Diagnosis, CHI_MLU, tokens_CHI), Diagnosis == "ASD")

test = select(clean, SUBJ,  Diagnosis, CHI_MLU, tokens_CHI, tokens_CHI)
#Only selects it once 
...
clean_new = clean_new %>%
  ...
  group_by(VISIT)%>%
  mutate(mean_tokens_visit = sum(tokens_CHI)/length(tokens_CHI))

clean_new = clean_new %>%
  group_by(SUBJ)%>%
  mutate(mean_tokens_chi = sum(tokens_CHI)/length(tokens_CHI))
